...
namespace NerdyDuck.ParameterValidation
{
	...
	public class ParameterValidator
	{
		...
		public IEnumerable<ParameterValidationResult> Validate(object value, ParameterDataType dataType, IReadOnlyList<Constraint> constraints, string memberName)
		{
			return Validate(value, dataType, constraints, memberName, null);
		}
		...
		/// <returns>An enumeration of <see cref="ParameterValidationResult"/>s. If the enumeration is empty, no validation error was found.</returns>
		public IEnumerable<ParameterValidationResult> Validate(object value, ParameterDataType dataType, IReadOnlyList<Constraint> constraints, string memberName, string displayName)
		{
			...
			{
				throw new CodedArgumentNullException(Errors.CreateHResult(0x9a), nameof(memberName));
			}
			...
			if (value == null)
			{
				...
				{
					ReturnValue.Add(new ParameterValidationResult(Errors.CreateHResult(0x9b), Properties.Resources.ParameterValidator_Validate_ValueNull, new string[] { memberName }, new Constraints.NullConstraint()));
				}
			}
			else
			{
		...
		/// <param name="constraints">A list of <see cref="Constraint"/>s to validate <paramref name="value"/> against.</param>
		/// <param name="memberName">The name of the property or control containing the <paramref name="value"/> to validate.</param>
		public void ValidateWithException(object value, ParameterDataType dataType, IReadOnlyList<Constraint> constraints, string memberName)
		{
			ValidateWithException(value, dataType, constraints, null);
		}
		...
		/// <param name="memberName">The name of the property or control containing the <paramref name="value"/> to validate.</param>
		/// <param name="displayName">The display name of the control that contains or displays the <paramref name="value"/> to validate. May be <see langword="null"/>.</param>
		public void ValidateWithException(object value, ParameterDataType dataType, IReadOnlyList<Constraint> constraints, string memberName, string displayName)
		{
			IEnumerable<ParameterValidationResult> Results = Validate(value, dataType, constraints, memberName, displayName);

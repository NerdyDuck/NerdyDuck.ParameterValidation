	...
	public class MaximumLengthConstraint : Constraint
	{
		...
		protected override void OnValidation(IList<ParameterValidationResult> results, object value, ParameterDataType dataType, string memberName, string displayName)
		{
			...
			if (CurrentLength > mMaximumLength)
			{
				if (IsString)
				{
					results.Add(new ParameterValidationResult(Errors.CreateHResult(0x38), string.Format(Properties.Resources.MaximumLengthConstraint_Validate_FailedString, mMaximumLength, CurrentLength), this));
				}
				else
				{
					results.Add(new ParameterValidationResult(Errors.CreateHResult(0x38), string.Format(Properties.Resources.MaximumLengthConstraint_Validate_FailedBytes, mMaximumLength, CurrentLength), this));
				}
			}
		}
		#endregion

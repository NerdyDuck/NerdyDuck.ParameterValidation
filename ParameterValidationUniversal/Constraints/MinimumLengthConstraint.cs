	...
	public class MinimumLengthConstraint : Constraint
	{
		...
		protected override void OnValidation(IList<ParameterValidationResult> results, object value, ParameterDataType dataType, string memberName, string displayName)
		{
			...
			if (CurrentLength < mMinimumLength)
			{
				if (IsString)
				{
					results.Add(new ParameterValidationResult(Errors.CreateHResult(0x39), string.Format(Properties.Resources.MinimumLengthConstraint_Validate_FailedString, mMinimumLength, CurrentLength), this));
				}
				else
				{
					results.Add(new ParameterValidationResult(Errors.CreateHResult(0x39), string.Format(Properties.Resources.MinimumLengthConstraint_Validate_FailedBytes, mMinimumLength, CurrentLength), this));
				}
			}
		}
		#endregion

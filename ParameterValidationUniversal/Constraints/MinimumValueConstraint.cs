...
namespace NerdyDuck.ParameterValidation.Constraints
{
	...
	public class MinimumValueConstraint : Constraint
	{
		...
		protected override void OnValidation(IList<ParameterValidationResult> results, object value, ParameterDataType dataType, string memberName, string displayName)
		{
				...
				if (IsConvertibleType(mDataType))
				{
					...
					else
					{
						...
						if (ConvertedValue == null)
						{
							...
							{
								throw new CodedArgumentException(Errors.CreateHResult(0x2), string.Format(Properties.Resources.Global_Validate_TypeNotConvertible, type.Name, ExpectedNetType.Name));
							}
							catch (OverflowException)
							{
								throw new CodedArgumentException(Errors.CreateHResult(0x2), string.Format(Properties.Resources.Global_Validate_TypeNotConvertible, type.Name, ExpectedNetType.Name));
			...
			if (CompareResult > 0)
			{
				results.Add(new ParameterValidationResult(Errors.CreateHResult(0x20), string.Format(Properties.Resources.MinimumValueConstraint_Validate_Failed, mMinimumValue, value), this));
			}
		}
		#endregion
		...
		/// </summary>
		/// <param name="value">The value to check.</param>
		/// <returns>The value, either in its origianl form, or converted.</returns>
		private object CheckValueType(object value)
		{
			...
			else if (ExpectedNetType == AltNetType)
			{
				AltMinimumValue = CheckedValue;
			}
			else
			{
				try
				...
				{
					throw new ParameterConversionException(Errors.CreateHResult(0x24), string.Format(Properties.Resources.Global_CheckValueType_AltNotConvertible, type.Name, ExpectedNetType.Name), mDataType, value, ex);
				}
				catch (FormatException ex)
				{
					throw new ParameterConversionException(Errors.CreateHResult(0x24), string.Format(Properties.Resources.Global_CheckValueType_AltNotConvertible, type.Name, ExpectedNetType.Name), mDataType, value, ex);

...
namespace NerdyDuck.ParameterValidation.Constraints
{
	...
	public class AllowedSchemeConstraint : Constraint
	{
			...
			: base(AllowedSchemeConstraintName)
		{
			...
			{
				throw new CodedArgumentNullException(Errors.CreateHResult(0x0d), nameof(scheme));
			}
		...
		protected override void OnValidation(IList<ParameterValidationResult> results, object value, ParameterDataType dataType, string memberName, string displayName)
		{
			...
			if (!SchemeFound)
			{
				results.Add(new ParameterValidationResult(Errors.CreateHResult(0x11), string.Format(Properties.Resources.AllowedSchemeConstraint_Validate_Failed, Scheme, ConcatSchemes()), this));
			}
		}
		#endregion
		#endregion
		...
		private void CheckSchemes(IReadOnlyList<string> schemes)
		{
			if (schemes.Count == 0)
			{

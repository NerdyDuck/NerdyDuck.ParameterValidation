...
namespace NerdyDuck.ParameterValidation.Constraints
{
	...
	public class MaximumValueConstraint : Constraint
	{
		...
		protected override void OnValidation(IList<ParameterValidationResult> results, object value, ParameterDataType dataType, string memberName, string displayName)
		{
			...
			if (CompareResult < 0)
			{
				results.Add(new ParameterValidationResult(Errors.CreateHResult(0x18), string.Format(Properties.Resources.MaximumValueConstraint_Validate_Failed, mMaximumValue, value), this));
			}
		}
		#endregion
		...
		private object CheckValueType(object value)
		{
			...
			else
			{
				...
				{
					throw new ParameterConversionException(Errors.CreateHResult(0x1d), string.Format(Properties.Resources.Global_CheckValueType_AltNotConvertible, type.Name, ExpectedNetType.Name), mDataType, value, ex);
				}
				catch (FormatException ex)
				{
					throw new ParameterConversionException(Errors.CreateHResult(0x1d), string.Format(Properties.Resources.Global_CheckValueType_AltNotConvertible, type.Name, ExpectedNetType.Name), mDataType, value, ex);

	...
	public class LengthConstraint : Constraint
	{
		...
		protected override void OnValidation(IList<ParameterValidationResult> results, object value, ParameterDataType dataType, string memberName, string displayName)
		{
			...
			if (CurrentLength != mLength)
			{
				if (IsString)
				{
					results.Add(new ParameterValidationResult(Errors.CreateHResult(0x37), string.Format(Properties.Resources.LengthConstraint_Validate_FailedString, mLength, CurrentLength), this));
				}
				else
				{
					results.Add(new ParameterValidationResult(Errors.CreateHResult(0x37), string.Format(Properties.Resources.LengthConstraint_Validate_FailedBytes, mLength, CurrentLength), this));
				}
			}
		}
		#endregion
